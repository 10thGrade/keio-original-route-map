...
- `resetView()`: ビューをリセット
- `toggleSidebar()`: 駅ランク設定パネル(モバイルではボトムシート)の開閉切り替え
- `toggleMobileMenu()`: CSV・画像出力メニュー(モバイル)の開閉切り替え
...
- **地図の拡大・縮小**: 2本指でのピンチ操作
- **地図の移動**: 1本指でのドラッグ操作
- **駅ランク設定**: 画面下部のハンドルバーをタップすると、駅ランク設定パネルが展開する。路線グループ
  ごとに折りたたみ(アコーディオン)表示になっており、タップで開閉できる
- **CSV・画像出力**: ヘッダー右上のメニュー(☰)ボタンから、CSV読込・CSV保存・SVG保存・PNG保存を
  まとめて操作できる
